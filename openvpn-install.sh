...
LimitNPROC=infinity" > /etc/systemd/system/openvpn-server@server.service.d/disable-limitnproc.conf
	...
	if [[ "$os" = "debian" || "$os" = "ubuntu" ]]; then
		apt-get update
		apt-get install -y --no-install-recommends openvpn openssl ca-certificates jq $firewall
	elif [[ "$os" = "centos" ]]; then
		yum install -y epel-release
		yum install -y openvpn openssl ca-certificates tar jq $firewall
	else
		# Else, OS must be Fedora
		dnf install -y openvpn openssl ca-certificates tar jq $firewall
	fi
...
set_var EASYRSA_CRL_DAYS ${crl_validity}
	...
	if [[ -n "$ca_dn" ]]; then
		echo "set_var EASYRSA_DN \"${ca_dn}\"" >> /etc/openvpn/server/easy-rsa/vars
		if [[ "$ca_dn" == "org" ]]; then
